...
const PreMeetingScreen = ({
    ...
    const displayRoomName = React.useMemo(() => {
        if (!_roomName) {
            return '';
        }

        // Check if this is a teacher room (format: teacher-xxxxxxxx)
        if (_roomName.toLowerCase().startsWith('teacher')) {
            try {
                const teacherFirstName = localStorage.getItem('teacherFirstName');
                const teacherRoomId = localStorage.getItem('teacherRoomId');
                
                // If we have teacher's name and this matches their room
                if (teacherFirstName && teacherRoomId && _roomName.toLowerCase() === teacherRoomId.toLowerCase()) {
                    return `Teacher ${teacherFirstName}`;
                }
                
                // Fallback: just say "Teacher's Classroom"
                return "Teacher's Classroom";
            } catch (error) {
        ...
        // For non-teacher rooms, return the original name
        return _roomName;
    }, [ _roomName ]);
                ...
                <div className = { classes.content }>
                    ...
                    <div className = { classes.contentControls }>
                        <div className = { classes.paddedContent }>
                            ...
                            {_roomName && !_roomName.toLowerCase().startsWith('teacher') && (
                                <span className = { classes.roomNameContainer }>
                                    {isOverflowing ? (
                                        <Tooltip content = { displayRoomName }>
                                            <span
